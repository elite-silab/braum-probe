...
3. 首次登录后可在「系统设置」→「账号安全」修改密码；修改成功后使用新密码重新登录。
4. 进入「VPS 节点」并添加节点，只需填写名称。
5. 复制后台生成的安装命令，在被监控 VPS 上执行。
6. 等待约一分钟，节点会开始上报资源和探测数据。

安装完成后，在 VPS 执行 `sudo braum-agentctl` 即可通过数字菜单查看状态和日志、启停、在线更新或卸载 Agent，不需要记忆 systemd 命令。

### 可选：添加网站/DNS 网络探测

资源监控安装后会自动生效；网络探测是另一项可选功能，用来检查“从某台 VPS 访问网站或 DNS”的延迟和可用率。完整流程只有三步：

1. 在后台「监控目标」创建网站 URL 或 DNS 域名。
2. 创建成功后，把目标分配给需要发起检查的 VPS 节点。
3. 等待 Agent 下一次心跳和首次检查结果。

只创建目标但没有分配节点时，不会产生探测结果；这不会影响 CPU、内存、磁盘、流量和运行时间监控。
...
| [版本发布指南](docs/版本发布指南.md) | 网站与 Agent 的独立版本、标签和 Release 流程 |
