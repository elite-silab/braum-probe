...
describe('Linux Agent 数字菜单管理脚本', () => {
  it('提供完整生命周期操作并安全执行在线更新', () => {
    ...
    expect(script).toContain('0. 退出')
    expect(script).not.toContain('10. 退出')
    expect(script).toContain("read -r -p '请选择 [0-9]：'")
    expect(script).toContain("curl --proto '=https' --tlsv1.2")
    expect(script).toContain('sha256sum -c')
    expect(script).toContain('systemctl is-active --quiet "$SERVICE_NAME"')
    expect(script).toContain('new_version="$(read_agent_version "$new_binary")"')
    expect(script.indexOf('install -m 0755 "$work_dir/$artifact" "$new_binary"'))
      .toBeLessThan(script.indexOf('new_version="$(read_agent_version "$new_binary")"'))
    expect(script).toContain('[[ "$new_version" != "braum-agent "* ]]')
    expect(script).toContain('无法识别下载文件的 Agent 版本，已取消更新。')
    expect(script).toContain('NoNewPrivileges=true')
    expect(script).toContain('systemd 安全配置已同步更新。')
