...
export default async function NodePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <nav className="mb-6 flex min-w-0 items-center gap-2 text-sm text-slate-500 dark:text-slate-400" aria-label="面包屑导航"><a href="/" className="shrink-0 hover:text-slate-900 dark:hover:text-white">总览</a><span>/</span><span className="truncate text-slate-900 dark:text-white">{node.name || `节点 ${id}`}</span></nav>

      <header className="public-status-hero relative mb-6 overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-7">
        <div className="pointer-events-none absolute -right-16 -top-20 h-48 w-48 rounded-full bg-brand-400/10 blur-3xl" aria-hidden="true" />
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="relative min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-600 dark:text-brand-400">Server details</p><h1 className="mt-2 break-words text-2xl font-bold tracking-tight text-slate-950 dark:text-white sm:text-3xl">{node.name}</h1><p className="mt-2 truncate text-xs text-slate-400" title={id}>节点 ID · {id}</p></div>
          <div className="relative flex items-center gap-2 rounded-full border border-slate-200/80 bg-white/80 px-3 py-1.5 text-sm font-medium shadow-sm dark:border-slate-700 dark:bg-slate-800/80"><StatusDot status={node.status === 'active' ? 'online' : node.status === 'offline' ? 'offline' : 'degraded'} size="sm" /><span>{agent?.registration_status === 'pending' ? '等待安装探针' : statusMap[node.status] || node.status}</span></div>
        </div>
        <div className="relative mt-6 grid grid-cols-2 gap-x-4 gap-y-5 border-t border-slate-200/70 pt-5 dark:border-slate-700/70 lg:grid-cols-4"><Info label="主机名" value={agent?.hostname || '尚未上报'} /><Info label="区域" value={node.country === '待识别' ? '安装后自动识别' : regionLabels[node.region] || node.region} /><Info label="位置" value={node.city === '待识别' ? '安装后自动识别' : [node.city, node.country].filter(Boolean).join(', ') || '--'} /><Info label="检查间隔" value={`${node.probe_interval || 60} 秒`} /></div>
      </header>

      <section className="mb-6">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-semibold text-slate-900 dark:text-white">资源使用</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{latestMetrics?.collected_at ? `采集于 ${formatDate(latestMetrics.collected_at)}` : '等待探针首次上报'}</p></div>{agent?.agent_version && <span className="rounded-full bg-brand-50 px-3 py-1 text-xs text-brand-700 dark:bg-brand-950/40 dark:text-brand-300">探针 {agent.agent_version}</span>}</div>
        ...
        </div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-8 text-center dark:border-slate-700 dark:bg-slate-900/70 sm:p-10"><p className="font-medium text-slate-700 dark:text-slate-200">还没有收到资源数据</p><p className="mt-1 text-sm leading-6 text-slate-500">管理员完成探针安装后，CPU、内存、磁盘和网络数据会显示在这里。</p></div>}
      </section>
      ...
      <div className="card mb-6"><div className="mb-4"><h2 className="text-lg font-semibold text-slate-900 dark:text-white">最近网络检查</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">从这台服务器访问监控目标的最新结果。</p></div>{probeResults.length ? <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800"><tr>{['目标', '状态', '延迟', 'HTTP 状态', '时间'].map((label) => <th key={label} className="px-4 py-3 font-medium text-slate-500 dark:text-slate-400">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-700">{probeResults.slice(0, 20).map((probe) => <tr key={probe.id || `${probe.target_id}-${probe.probe_at}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/50"><td className="px-4 py-3 text-slate-700 dark:text-slate-300">{probe.target_name || probe.target_id?.slice(0, 8) || '--'}</td><td className="px-4 py-3"><span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${probe.success ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>{probe.success ? '正常' : '失败'}</span></td><td className="px-4 py-3 text-slate-700 dark:text-slate-300">{probe.latency_ms ? `${probe.latency_ms}ms` : '--'}</td><td className="px-4 py-3 text-slate-500 dark:text-slate-400">{probe.status_code || '--'}</td><td className="px-4 py-3 text-slate-500 dark:text-slate-400">{probe.probe_at ? formatDate(probe.probe_at) : '--'}</td></tr>)}</tbody></table></div> : <div className="py-6 text-center"><p className="font-medium text-slate-600 dark:text-slate-300">还没有网络检查记录</p><p className="mt-1 text-sm text-slate-400 dark:text-slate-500">配置监控目标后，检查结果会显示在这里。</p></div>}</div>

      <div className="card"><h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-white">网络可用率</h2><div className="space-y-3"><AvailabilityRow label="24 小时" {...availability.hours24} /><AvailabilityRow label="7 天" {...availability.days7} /><AvailabilityRow label="30 天" {...availability.days30} /></div></div>
    </div>
  )
}

function MissingNode({ id }: { id: string }) {
  return <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><div className="card py-12 text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500"><svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="6" rx="2" /><rect x="3" y="14" width="18" height="6" rx="2" /><path d="M8 7h.01M8 17h.01" /></svg></span><h1 className="mt-4 text-lg font-semibold text-slate-900 dark:text-white">找不到这台服务器</h1><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">节点 {id} 可能已被删除，或当前暂时无法访问。</p><a href="/" className="mt-5 inline-flex rounded-xl bg-brand-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-brand-700">返回状态总览</a></div></div>
}

function Info({ label, value }: { label: string; value: string }) { return <div className="min-w-0"><p className="text-xs text-slate-500 dark:text-slate-400">{label}</p><p className="mt-1 break-words text-sm font-medium text-slate-900 dark:text-white">{value}</p></div> }
