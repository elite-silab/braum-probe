...
- hostname、Linux 完整发行版名称、内核、CPU 架构和 Agent 版本；
- CPU、内存、Swap、磁盘和系统负载；
...
首页节点卡片会直接展示完整发行版、架构、CPU、内存和磁盘使用量、本次开机累计下载/上传/总流量、最近采样区间的上下行速率，以及精确到秒的运行时间，不需要进入节点详情。VPS 重启后 Linux 网络计数器和运行时间会重新从零开始，这不是数据丢失。

## 6. 添加 HTTP/DNS 探测

服务器资源监控安装后立即生效，网络探测是可选功能。网络探测检查的是“从该 VPS 访问目标”的质量；节点在线不代表已经配置了网络探测。

1. 在后台打开「监控目标」。
2. 添加 HTTP URL 或 DNS 域名。
3. 创建成功后，在分配步骤勾选需要执行检查的节点；也可以稍后编辑节点进行关联。
4. 等待下一次心跳和首次探测结果。

只创建目标但未关联节点时，目标会保存在后台，但不会下发给任何 Agent，也不会产生探测结果。这不会影响资源、流量和运行时间上报。
...
3. 校验成功后原子替换程序并重启服务；
4. 同步最新版 systemd 安全配置；
5. 启动失败时同时恢复旧程序和旧服务配置；
6. 同步最新版管理脚本。

在线更新不会修改 `/etc/braum-agent/config.json`，因此节点 ID 和长期密钥都会保留，不需要在后台重新生成安装命令。

更新结果会显示升级前后的完整版本号。再次打开菜单并选择「4. 查看版本与安全配置」，应看到当前 Agent 版本，以及 `NoNewPrivileges=yes`、`ProtectSystem=strict` 和 `ProtectHome=yes`。
